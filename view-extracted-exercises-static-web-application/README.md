...
3. The file is saved in IndexedDB in the browser.
...
9. Use `clear all data` to remove the saved archive and return to the upload screen.
10. If the app detects archive data from the older `localStorage` version, it wipes that data and asks the user to re-upload.
...
- Data is stored locally in the browser only, using IndexedDB.
- Legacy `localStorage` archives are intentionally deleted on startup; users must re-upload once after upgrading.
