...
describe("buildInstructorExport", () => {
  ...
  it("matches the instructor's submissions using the same exercise id normalization as the Meteor app", () => {
    const exportData = buildInstructorExport({
      instructor: {
        _id: "instructor-1",
        emails: [{ address: "teacher@example.com" }]
      ...
      exerciseSets: [
        {
          ...
          lectures: [
            {
              name: "lecture_01",
              units: [{ name: "unit-1", rawExercises: ["/ex/proof/from/A and B/to/A/"] }]
            }
          ]
        }
      ],
      submissions: [
        {
          _id: "submission-1",
          owner: "instructor-1",
          exerciseId: "/ex/proof/from/A%20and%20B/to/A"
        },
        {
          _id: "submission-2",
          owner: "other-user",
